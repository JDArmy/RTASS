...
进攻水平由情报侦察能力[IRC]、武器化能力[WC]、漏洞挖掘利用能力[VMEC]、抗威胁检测能力[ATDC]、社会工程能力[SEC]、权限维持能力[PPC]、隧道搭建能力[TCC]、横向移动能力[LMC]、AI辅助攻击能力[AIOC]、供应链攻击能力[SCAC]、云原生攻击能力[CNAC] 11个能力项组成。计算方法为：

> 进攻水平[OL] = ( IRC * 1 + WC * 1 + VMEC * 1 + ATDC * 1 + SEC * 1 + PPC * 1 + TCC * 1 + LMC * 1 + AIOC * 1 + SCAC * 1 + CNAC * 1 )/11
...
AI辅助攻击能力主要评估攻击者利用AI/LLM技术辅助攻击的能力，包括AI生成钓鱼内容、漏洞挖掘、自动化渗透等

- 0 - 不使用AI辅助攻击手段
- 1 - 使用AI工具辅助生成钓鱼内容或简单攻击脚本
- 2 - 使用AI辅助漏洞分析、代码生成或自动化渗透测试
- 3 - 使用AI进行深度伪造社工、智能化漏洞挖掘或自适应攻击规避
- 4 - 能够开发或使用AI驱动的自主攻击系统，实现高度自动化的APT行动

##### **供应链攻击能力[SCAC]**

供应链攻击能力主要评估攻击者通过软件供应链、第三方组件或开发工具链实施攻击的能力

- 0 - 无供应链攻击能力
- 1 - 能够识别目标的软件供应链依赖关系并发现已知漏洞
- 2 - 能够利用已知的开源组件或第三方库漏洞实施供应链攻击
- 3 - 能够污染第三方软件包、开发工具链或CI/CD流水线实施攻击
- 4 - 能够渗透目标核心供应商或开发环境，实施高级持久性供应链攻击（如SolarWinds级别）

##### **云原生攻击能力[CNAC]**

云原生攻击能力主要评估攻击者针对云基础设施、容器、K8s及云服务配置错误的攻击能力

- 0 - 无云原生攻击能力
- 1 - 能够利用云服务配置错误（如S3公开桶、开放安全组）获取初始访问
- 2 - 能够进行容器逃逸、K8s权限提升或云凭证窃取
- 3 - 能够横向移动至云控制平面，获取云账号高权限或跨账号攻击
- 4 - 能够在多云环境中实现持久化，对云基础设施造成重大影响或数据大规模泄露
...
防护水平由日志事件管理能力[IEMC]、软件更新管理能力[SUMC]、威胁识别阻断能力[TIBC]、基线安全管理能力[BSMC]、账号安全管理能力[ASMC]、权限安全管理能力[PSMC]、网络访问控制能力[NACC]、零信任架构能力[ZTAC]、云安全管理能力[CSMC]、API安全能力[APSC] 10个能力项组成。计算方法为：

> 防护水平[PL] = ( IEMC * 1 + SUMC * 1 + TIBC * 1 + BSMC * 1 + ASMC * 1 + PSMC * 1 + NACC * 1 + ZTAC * 1 + CSMC * 1 + APSC * 1 )/10
...
零信任架构能力主要评估企业零信任安全架构的建设与落地水平，包括身份验证、最小权限、持续验证等

- 0 - 无零信任架构建设，依赖传统边界安全
- 1 - 有基本的身份验证强化和最小权限原则落地
- 2 - 实现了网络微隔离和基于身份的持续验证机制
- 3 - 建立了覆盖用户、设备和应用的完整零信任访问控制体系
- 4 - 实现了动态自适应的零信任架构，具备持续风险评估和自动化策略调整能力

##### **云安全管理能力[CSMC]**

云安全管理能力主要评估企业云安全态势管理（CSPM）、云工作负载保护（CWPP）及云原生应用保护（CNAPP）能力

- 0 - 无云安全管理能力，云资产缺乏安全管控
- 1 - 有基础的云资产梳理和访问控制配置
- 2 - 有云安全态势管理（CSPM）能力，能够发现和修复云配置错误
- 3 - 有云工作负载保护（CWPP）能力，覆盖容器和无服务器安全
- 4 - 有完整的云原生应用保护平台（CNAPP）能力，实现云安全全生命周期管理

##### **API安全能力[APSC]**

API安全能力主要评估企业API资产发现、风险评估、运行时防护及全生命周期安全管理能力

- 0 - 无API安全管理能力，API暴露面不清晰
- 1 - 有基础的API认证鉴权机制（如OAuth2.0、API Key）
- 2 - 有API网关和基础的API安全防护能力（限流、WAF）
- 3 - 有API资产发现、风险评估和运行时异常行为检测能力
- 4 - 有完善的API安全全生命周期管理能力，覆盖设计规范、安全测试、运行时防护和监控

### **监测水平[DL]**

监测水平由外部威胁流量感知[ETTA]、外部鱼叉攻击感知[EHAA]、近源渗透攻击感知[PPAA]、内网威胁流量感知[ITTA]、端点恶意软件感知[EMA]、端点异常行为感知[EABA]、威胁检出率[TDR]、威胁鉴别及时率[TITR]、扩展检测响应能力[XDRC]、威胁情报平台能力[TIPC] 10个能力项组成。计算方法为：

> 监测水平[DL] = ( ETTA * 1 + EHAA * 1 + PPAA * 1 + ITTA * 1 + EMA * 1 + EABA * 1 + TDR * 1 + TITR * 1 + XDRC * 1 + TIPC * 1 )/10
...
扩展检测响应能力主要评估企业XDR/SOAR平台的建设水平，实现跨端点、网络、云的统一检测与自动化响应

- 0 - 无XDR/SOAR能力，安全工具孤立运行
- 1 - 有基础的SIEM日志聚合与规则告警能力
- 2 - 有EDR与网络流量分析联动的检测能力
- 3 - 有跨端点、网络、云的统一XDR检测与响应能力
- 4 - 有完善的SOAR自动化编排响应能力，实现威胁的自动化处置闭环

##### **威胁情报平台能力[TIPC]**

威胁情报平台能力主要评估企业威胁情报的生产、消费与运营能力，包括情报源接入、IOC联动和攻击者画像

- 0 - 无威胁情报能力
- 1 - 订阅了基础的商业或开源威胁情报源（如MISP、OpenCTI）
- 2 - 能够将威胁情报与安全设备联动，实现IOC自动封堵
- 3 - 有内部威胁情报生产能力，能够对攻击者进行TTP画像分析
- 4 - 有完善的威胁情报平台，实现情报的生产、消费、共享与反馈闭环
...
参考CVSS评分框架，RTASS同样使用”向量字符串”来记录评分过程，以及对RTASS指标信息进行传输。RTASS向量字符串以标签”RTASS:”和当前版本的数字表示（譬如：1.0.5）开头。指标信息以一组指标的形式出现，每个指标前面都有一个正斜杠”/”，作为分隔符。每个指标都是缩写形式的指标名称、冒号及指标值构成。缩写形式在本规范的前面定义（在每个因子名称后面的括号中），并在下表中进行了总结。
...
RTASS:1.0.5/OL:2/OD:0/TR:0/AL:4/PL:1/DL:1/RL:0/TL:4/VD:2/VE:3/VL:2/VDW:2/LC:0/LI:4/LA:2/FD:4/RD:4/CI:3/DLC:1/OLC:2/ESA:1

RTASS Plus模式向量字符串示例如下：
RTASS:1.0.5/OL:2/OD:2/TR:2/AL:1/PL:4/DL:4/RL:0/TL:2/VD:4/VE:3/VL:3/VDW:2/LC:0/LI:0/LA:0/FD:1/RD:0/CI:2/DLC:4/OLC:4/ESA:4/IRC:1/WC:3/VMEC:2/ATDC:3/SEC:3/PPC:1/TCC:3/LMC:0/AIOC:1/SCAC:1/CNAC:1/GAT:4/GAC:3/GAA:3/IEMC:0/SUMC:4/TIBC:1/BSMC:1/ASMC:3/PSMC:4/NACC:4/ZTAC:2/CSMC:2/APSC:1/ETTA:1/EHAA:3/PPAA:0/ITTA:2/EMA:3/EABA:0/TDR:1/TITR:3/XDRC:2/TIPC:1/THC:2/TDTR:2/TAC:1/IPTC:1/ATCC:3/DEC:4
...
- https://www.first.org/cvss/v4.0/specification-document, FIRST CVSSv4, CVSSv4.0规范文档（2023年发布）
- https://attack.mitre.org/, MITRE ATT&CK, MITRE ATT&CK企业级攻击战术与技术知识库
- https://owasp.org/www-project-api-security/, OWASP API Security, OWASP API安全Top 10（2023版）
- https://www.nist.gov/publications/zero-trust-architecture, NIST, NIST SP 800-207 零信任架构标准
