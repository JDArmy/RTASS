...
import LANG from "@/i18n/cn/doc.json";
///////////////////////////////////////////////////////////////////////////////
...
>
> ${LANG.maintainer}: ${RTASS.maintainer}, ${
    LANG.collaborators
  }: ${Object.values(RTASS.thanks)
    .map((t) => t.name)
    .join(", ")}
>
...
${Object.values(RTASS.factorCategories)
  .map((fc) => {
    let fcDoc = "";
    fcDoc += fc.name + fc.description;
    fcDoc += vm.runInNewContext(LANG.categoryDesc, { RTASS }) + "\n\n";
    fcDoc += RTASS.factorCategories.CF.processScores
      .map(
        (ps) =>
          "- " +
          RTASS.processScores[ps].name +
          RTASS.processScores[ps].description
      )
      .join("\n");
    return fcDoc;
  })
...
${Object.keys(RTASS.finalScores)
  .map((fsKey) => {
    ...
    doc +=
      vm.runInNewContext(LANG.finalScoresDesc, { RTASS, fs, fsKey }) + "\n\n";
    //最终分值算法公式
    doc += "> " + fs.name + ": " + fsKey + " = (";
    doc += Object.values(fs.vectors)
      .map((v) => v.algorithm)
      .join("+");
...
${Object.values(RTASS.factorCategories)
  .map((fc) => {
    ...
    //因子
    doc += fc.processScores
      .map((ps) => {
        let psDoc = "";
        psDoc +=
          "#### **" + RTASS.processScores[ps].name + "[" + ps + "]**\n\n";

        let psVectors = RTASS.processScores[ps].vectors;
        psDoc +=
          vm.runInNewContext(LANG.processScoringDesc, {
            RTASS,
            ps,
            psVectors,
          }) + "\n\n";
        ...
        psDoc += Object.values(psVectors)
          .map((v) => v.algorithm)
          .join("+");
        psDoc += ")/" + Object.values(psVectors).length + "\n\n";

        //因子介绍
        psDoc += Object.keys(psVectors)
          .map((fkey) => {
            let fDoc = "";
...
${Object.keys(RTASS.factors)
  .map((fKey) => {
    ...
    if (f.vectors !== undefined) {
      ...
      //算法
      bDoc += "> " + f.name + "[" + fKey + "] = (";
      bDoc += Object.values(f.vectors)
        .map((v) => v.algorithm)
        .join("+");
      bDoc += ")/" + Object.values(f.vectors).length + "\n\n";
      //算法介绍
      bDoc +=
        Object.values(f.vectors)
          .map((v) => v.description)
          .join("") + "\n\n";
      //能力介绍
      bDoc += Object.keys(f.vectors)
        .map((aKey) => {
          let aDoc = "";
...
RTASS:${RTASS.version}/${Object.keys(RTASS.factors)
    .map((fKey) => {
      let fDoc = "";
...
${Object.values(RTASS.references)
  .map((r) => "- " + r.url + ", " + r.name + ", " + r.description)
  .join("\n")}
